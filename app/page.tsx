...
const LandingPage = () => {
    ...
    if (sent) {
        return (
            <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6 text-center animate-in fade-in zoom-in-95">
                <div className="w-16 h-16 bg-emerald-100 text-emerald-600 rounded-full flex items-center justify-center mb-6"><Check size={32} strokeWidth={3} /></div>
                <h2 className="text-2xl font-extrabold text-slate-900 mb-2">Check your email</h2>
                <p className="text-slate-500 font-medium max-w-sm">We sent a magic link to <span className="text-slate-900 font-bold">{email}</span>.</p>
                <button onClick={() => setSent(false)} className="mt-8 text-sm font-bold text-slate-400 hover:text-slate-600">Try a different email</button>
            </div>
        );
    }
